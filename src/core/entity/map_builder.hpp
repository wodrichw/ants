...
class BspListener : public ITCODBspCallback {
   ...
   public:
    BspListener(MapSectionData &section_data) : section_data(section_data), room_num(0) {
    ...
    bool visitNode(TCODBsp *node, void *user_data) {
        ...
        if(room_num != 0) {
            ...
            section_data.corridors.push_back(Rect::from_corners(lastx, lasty, x + w / 2, lasty));
            section_data.corridors.push_back(Rect::from_corners(x + w / 2, lasty, x + w / 2, y + h / 2));
        }
        lastx = x + w / 2;
        lasty = y + h / 2;
        room_num++;
...
struct RandomMapBuilder {
    Rect border;
    RandomMapBuilder(Rect const& border): border(border) {
        SPDLOG_INFO("Creating RandomMapBuilder");
    }
    void operator()(MapSectionData &section_data) const {
        SPDLOG_INFO("Building random section_data at {}x{} - max room size: {} max ratio: {}", section_data.border.w, section_data.border.h, ROOM_MAX_SIZE, 1.5f);
        section_data.border = border;

        TCODBsp bsp(0, 0, section_data.border.w, section_data.border.h);  // bsp is binary space partition tree
...
private:
    ...
    void load_file(const std::string &filename) {
        ...
        file >> read_mode;
        rect_parser = read_mode == "CENTER" ? Rect::from_center : (read_mode == "CORNERS" ? Rect::from_corners : Rect::from_top_left);
        ...
        for (long i = 0; i <  corridor_count; ++i) {
            ...
            if (vertical_gap && horizontal_gap) {
                ...
                corridors.push_back(Rect::from_corners(r1.center_x, r1.center_y, r2.center_x, r1.center_y));
                corridors.push_back(Rect::from_corners(r2.center_x, r1.center_y, r2.center_x, r2.center_y));
                ...
                corridors.push_back(Rect::from_corners(center_x, r1.center_y, center_x, r2.center_y));
                ...
                corridors.push_back(Rect::from_corners(r1.center_x, center_y, r2.center_x, center_y));
            } else {
                // overlap
