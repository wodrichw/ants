...
bool ButtonController::canCreateButton(ButtonController::ButtonData const& data)
{
...
ButtonController::Button* ButtonController::createButton(ButtonData const& data, std::function<bool()>onClick, std::optional<tcod::ColorRGB> color)
{
    long end_x = data.x + data.w, end_y = data.y + data.h;
...
{
    return canMoveButton(b->data.layer, b->data.x + dx, b->data.y + dy, b->data.w, b->data.h);
}
