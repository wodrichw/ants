...
void tcodRenderer::renderTextEditor(TextEditorHandler const& editor, size_t ant_count)
{
    ...
    for (int i = 0; i < editor.textBoxHeight + 2; ++i) {
        asciiGrid[i] = std::string(editor.regBoxWidth + editor.textBoxWidth + 4, ' ');
        asciiGrid[i][editor.regBoxWidth + editor.textBoxWidth + 3] = '\n';
    }

    Box mainBox(asciiGrid, 0, 0, editor.textBoxWidth + 2, editor.textBoxHeight + 2);
    ...
    tcod::print_rect(
            root_console,
            {0, 0, editor.textBoxWidth + editor.regBoxWidth + 4, editor.textBoxHeight + editor.regBoxHeight + 3},
